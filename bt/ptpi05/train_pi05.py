"""
PI0.5 Pre-Training on community data (LeRobot v3.0 format).

Standalone training script that:
  1. Loads a converted community dataset via LeRobotDataset (v3.0)
  2. Builds a PI05Policy (random init for pre-training, or from pretrained VLM)
...
Usage:
    ...
    python -m bt.ptpi05.train_pi05 \
        --repo-id community_pt_v30 --local-root data/community_pt_v30 \
        --no-pretrained --steps 20 --batch-size 2

    # Pre-train from pretrained base
    python -m bt.ptpi05.train_pi05 \
        --repo-id community_pt_v30 --local-root data/community_pt_v30 \
        --pretrained-path lerobot/pi05_base --steps 3000
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="PI05 pre-training on community data")

    # Dataset
    p.add_argument("--repo-id", default="community_pt_v30")
    ...
    p.add_argument("--local-root", default="data/community_pt_v30",
...
def train(args):
    ...
    if args.output_dir:
        ...
        out_dir = Path(f"outputs/pt_pi05/{dt.datetime.now():%Y-%m-%d_%H-%M-%S}")

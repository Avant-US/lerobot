...
def make_groot2_pre_post_processors(
...
def _build_eagle_processor(tokenizer_assets_repo: str = DEFAULT_TOKENIZER_ASSETS_REPO) -> ProcessorMixin:
    ...
    # `Eagle25VLProcessor` internally calls `image_processor(...)` without forwarding
    # `return_tensors`. On recent transformers/image-processor versions this defaults
    # to nested python lists and breaks downstream `.shape` access. Force tensor output.
    if hasattr(proc, "image_processor") and hasattr(proc.image_processor, "return_tensors"):
        proc.image_processor.return_tensors = "pt"
    return proc


@dataclass
@ProcessorStepRegistry.register(name="groot2_pack_inputs_v3")
...
@dataclass
@ProcessorStepRegistry.register(name="groot2_eagle_encode_v3")
...
@dataclass
@ProcessorStepRegistry.register(name="groot2_eagle_collate_v3")
...
@dataclass
@ProcessorStepRegistry.register(name="groot2_action_unpack_unnormalize_v1")
